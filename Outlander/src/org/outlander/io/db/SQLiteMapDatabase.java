...
public class SQLiteMapDatabase {
    ...
    public void setFile(final String aFileName) throws SQLiteException {
        ...
        mDatabase = new CacheDatabaseHelper(null, aFileName)
                .getWritableDatabase();
    ...
    public void updateMinMaxZoom() throws SQLiteException {
        if (mDatabase != null) {
            ...
            mDatabase
                    .execSQL("CREATE TABLE IF NOT EXISTS info AS SELECT MIN(z) AS minzoom, MAX(z) AS maxzoom FROM tiles");
        }
    }

    public/* synchronized */void putTile(final int aX, final int aY,
            final int aZ, final byte[] aData) {
        if (mDatabase != null) {
    ...
    public/* synchronized */byte[] getTile(final int aX, final int aY,
            final int aZ) {
        byte[] ret = null;

        if (mDatabase != null) {
            final Cursor c = mDatabase.rawQuery(
    ...
    public int getMaxZoom() {
        ...
        if (mDatabase != null) {
            final Cursor c = mDatabase.rawQuery(
                    "SELECT 17-minzoom AS ret FROM info", null);
            if (c != null) {
                if (c.moveToFirst()) {
    ...
    public int getMinZoom() {
        ...
        if (mDatabase != null) {
            final Cursor c = mDatabase.rawQuery(
                    "SELECT 17-maxzoom AS ret FROM info", null);
            if (c != null) {
                if (c.moveToFirst()) {
                    ret = c.getInt(c.getColumnIndexOrThrow("ret"));
                }
