...
public class DBManager {
    ...
    public void addPoi(final String title, final String descr, final GeoPoint point) {
        mGeoDatabase.addPoi(title, descr, point.getLatitude(), point.getLongitude(), DBConstants.ZERO, DBConstants.ZERO, DBConstants.ZERO, DBConstants.ZERO,
                R.drawable.poi);
    }

    public void updatePoi(final PoiPoint point) {
        if (point.getId() < 0) {
            mGeoDatabase.addPoi(point.getTitle(), point.getDescr(), point.getGeoPoint().getLatitude(), point.getGeoPoint().getLongitude(), point.getAlt(),
                    point.getCategoryId(), point.getPointSourceId(), point.isHidden() == true ? DBConstants.ONE : DBConstants.ZERO, point.getIconId());
        }
        else {
            mGeoDatabase.updatePoi(point.getId(), point.getTitle(), point.getDescr(), point.getGeoPoint().getLatitude(), point.getGeoPoint().getLongitude(),
                    point.getAlt(), point.getCategoryId(), point.getPointSourceId(), point.isHidden() == true ? DBConstants.ONE : DBConstants.ZERO,
                    point.getIconId());
        }
    }
    ...
    public List<PoiPoint> getPoiListForCategory(final int categoryid) {
        return doCreatePoiListFromCursor(mGeoDatabase.getPoiListForCategory(categoryid));
    }
    ...
    public List<PoiPoint> getPoiListNotHidden(final int zoom, final GeoPoint center, final double deltaX, final double deltaY) {
    ...
    public void addPoiStartActivity(final Context ctx, final GeoPoint touchDownPoint) {
        ctx.startActivity((new Intent(ctx, PoiActivity.class)).putExtra(DBConstants.LAT, touchDownPoint.getLatitude()).putExtra(DBConstants.LON,
    ...
    public PoiPoint getPoiPoint(final int id) {
        ...
        if (c != null) {
            if (c.moveToFirst()) {
                point = new PoiPoint(c.getInt(4), c.getString(2), c.getString(3), new GeoPoint((int) (1E6 * c.getDouble(0)), (int) (1E6 * c.getDouble(1))),
                        c.getInt(9), c.getInt(7), c.getInt(5), c.getInt(8), c.getInt(6));
            }
            c.close();
        }
    ...
    public PoiCategory getPoiCategory(final int id) {
        ...
        if (c != null) {
            if (c.moveToFirst()) {
                category = new PoiCategory(id, c.getString(0), c.getInt(2) == DBConstants.ONE ? true : false, c.getInt(3), c.getInt(4), c.getString(5));
            }
            c.close();
        }
    ...
    public List<PoiCategory> getPoiCategoryListFromCursor(final Cursor c) {
        ...
        if (c != null) {
            if (c.moveToFirst()) {
                do {
                    final PoiCategory category = new PoiCategory(c.getInt(1), c.getString(0), false, c.getInt(2), c.getInt(3), c.getString(4));
    ...
    public List<PoiCategory> getPoiCategoryList() {
        return getPoiCategoryListFromCursor(mGeoDatabase.getPoiCategoryListCursor());
    }

    public List<PoiCategory> getPoiUserCategoryList() {
        return getPoiCategoryListFromCursor(mGeoDatabase.getPoiUserCategoryListCursor());
    }

    public void updatePoiCategory(final PoiCategory poiCategory) {
        if (poiCategory.getId() < DBConstants.ZERO) {
            mGeoDatabase.addPoiCategory(poiCategory.Title, poiCategory.Hidden == true ? DBConstants.ONE : DBConstants.ZERO, poiCategory.IconId,
                    poiCategory.Descr);
        }
        else {
            mGeoDatabase.updatePoiCategory(poiCategory.getId(), poiCategory.Title, poiCategory.Hidden == true ? DBConstants.ONE : DBConstants.ZERO,
                    poiCategory.IconId, poiCategory.MinZoom, poiCategory.Descr);
        }
    }
    ...
    public void updateTrack(final Track track, final boolean dotransient) {
        ...
        if (track.getId() < 0) {
            trackId = (int) mGeoDatabase.addTrack(track.Name, track.Descr, track.Show ? DBConstants.ONE : DBConstants.ZERO, track.AvgSpeed, track.Distance,
                    track.Time);
        }
        else {
            trackId = track.getId();
            ...
            mGeoDatabase.updateTrack(trackId, track.Name, track.Descr, track.Show ? DBConstants.ONE : DBConstants.ZERO, track.AvgSpeed, track.Distance,
                    track.Time);
        }
        if (dotransient) {
            for (final TrackPoint trackpoint : track.getPoints()) {
                mGeoDatabase.addTrackPoint(trackId, trackpoint.getLatitude(), trackpoint.getLongitude(), trackpoint.alt, trackpoint.speed, trackpoint.date);
            }
        }
    }
    ...
    public List<Track> getTracksChecked() {
        ...
        if (c != null) {
            if (c.moveToFirst()) {
                ...
                do {
                    final Track track = new Track(c.getInt(3), c.getString(0), c.getString(1), c.getInt(2) == DBConstants.ONE ? true : false);
                    if (track != null) {
                        addTrackPointsToTrack(track);
                    }
                    trackList.add(track);
    ...
    public RouteCategory getTrackCategory(final int id) {
        ...
        if (c != null) {
            if (c.moveToFirst()) {
                category = new RouteCategory(id, c.getString(0), c.getString(1), c.getInt(2) == DBConstants.ONE ? true : false, c.getInt(3), c.getInt(4));
            }
            c.close();
        }
        return category;
        ...
        if (c != null) {
            if (c.moveToFirst()) {
                ...
                do {
                    ...
                    final RouteCategory routeCategory = new RouteCategory(c.getInt(2), c.getString(0), c.getString(1), false, -1, -1);
    ...
    public void updateRoute(final Route route, final boolean dotransient) {
        ...
        if (route.getId() < 0) {
            routeId = mGeoDatabase.addRoute(route.getName(), route.getDescr(), route.isShow() ? DBConstants.ONE : DBConstants.ZERO, route.getCategory());
        }
        else {
            routeId = route.getId();
            ...
            mGeoDatabase.updateRoute((int) routeId, route.getName(), route.getDescr(), route.isShow() ? 1 : 0, route.getCategory());
        }
        if (dotransient) {
            for (final PoiPoint point : route.getPoints()) {
                mGeoDatabase.addRoutePoi(point.getTitle(), point.getDescr(), point.getGeoPoint().getLatitude(), point.getGeoPoint().getLongitude(),
                        point.getAlt(), 0, (int) routeId, DBConstants.ONE, 0);
            }
        }
    }
    ...
    public Route getRoute(final int id) {
        ...
        if (c != null) {
            if (c.moveToFirst()) { // name, descr, show,categoryid ,routeid
                ...
                route = new Route(id, c.getString(0), c.getString(1), c.getInt(2) == DBConstants.ONE ? true : false, c.getInt(3));
                //
                //
    ...
    public Route getRouteByName(final String name) {
        ...
        if (c != null) {
            if (c.moveToFirst()) { // name, descr, show,categoryid ,routeid
                ...
                route = new Route(c.getInt(4), c.getString(0), c.getString(1), c.getInt(2) == DBConstants.ONE ? true : false, c.getInt(3));
            }
            c.close();
            c = null;
            if (route != null) {
        ...
        if (c != null) {
            if (c.moveToFirst()) {
                do {
                    ...
                    route.addRoutePoint(c.getString(2), c.getString(3), new GeoPoint((int) (c.getDouble(0) * 1E6), (int) (c.getDouble(1) * 1E6)), c.getInt(9),
                            0, c.getDouble(5), 0);
                } while (c.moveToNext());
            }
            c.close();
    ...
    public Route getRouteChecked() {
        ...
        if (c != null) {
            if (c.moveToFirst()) {
                route = new Route(c.getInt(2), c.getString(0), c.getString(1), c.getInt(4) == DBConstants.ONE ? true : false, c.getInt(3));
                //
                //
    ...
    public void updateRouteCategory(final RouteCategory routeCategory) {
        if (routeCategory.getId() < DBConstants.ZERO) {
            mGeoDatabase.addRouteCategory(routeCategory.Title, routeCategory.Description, routeCategory.Hidden == true ? DBConstants.ONE : DBConstants.ZERO,
                    routeCategory.IconId, routeCategory.MinZoom);
        }
        else {
            mGeoDatabase.updateRouteCategory(routeCategory.getId(), routeCategory.Title, routeCategory.Description,
                    routeCategory.Hidden == true ? DBConstants.ONE : DBConstants.ZERO, routeCategory.IconId, routeCategory.MinZoom);
        }
    }

    // proximity Target

    public void addProximityTarget(final String title, final String descr, final GeoPoint point) {
        mGeoDatabase.addPoi(title, descr, point.getLatitude(), point.getLongitude(), DBConstants.ZERO, DBConstants.ZERO, DBConstants.ZERO, DBConstants.ZERO,
                R.drawable.poi);
    }
    ...
    public void updateProximityTarget(final ProximityTarget point) {
        if (point.getId() < 0) {
            mGeoDatabase.addProximityTarget(point.Title, point.Descr, point.GeoPoint.getLatitude(), point.GeoPoint.getLongitude(), point.RouteId,
                    point.RouteOrder, point.Distance, point.enterOrLeave == true ? DBConstants.ONE : DBConstants.ZERO);
        }
        else {
            mGeoDatabase.updateProximityTarget(point.getId(), point.Title, point.Descr, point.GeoPoint.getLatitude(), point.GeoPoint.getLongitude(),
                    point.RouteId, point.RouteOrder, point.Distance, point.enterOrLeave == true ? DBConstants.ONE : DBConstants.ZERO);
        }
    }

    private List<ProximityTarget> doCreateProximityTargetListFromCursor(final Cursor c) {
        final ArrayList<ProximityTarget> items = new ArrayList<ProximityTarget>();
        if (c != null) {
            if (c.moveToFirst()) {
                do {
                    items.add(new ProximityTarget(c.getInt(0), c.getString(1), c.getString(2), new GeoPoint((int) (1E6 * c.getDouble(3)), (int) (1E6 * c
                            .getDouble(4))), c.getInt(5), c.getInt(6), c.getInt(7), c.getInt(8)));
                } while (c.moveToNext());
            }
            c.close();
        }
    ...
    public List<ProximityTarget> getProximityTargetList() {
        return doCreateProximityTargetListFromCursor(mGeoDatabase.getProximityTargetListCursor());
    }
