...
public abstract class FilteringLocationBroadcastReceiver extends LocationBroadcastReceiver {
    ...
    public void onLocationChanged(final Context context, final Location location) {
        final LocationPoint lastPoint = CachedLocationsDatabaseHelper.getInstance(context).retrieveLatestLocationPoint();
        if (lastPoint == null) {
            ...
            final LocationPoint point = CachedLocationsDatabaseHelper.getInstance(context).retrieveLatestLocationPoint();

            // True IFF accuracy is greater, but limited to 10% of the previous
            // accuracy and new point was generated by the same provider
            final float accuracyDifference = Math.abs(previousAccuracy - currentAccuracy);
            final boolean lowerAccuracyAcceptable = (currentAccuracy > previousAccuracy) && lastPoint.getProvider().equals(location.getProvider())
                    && (accuracyDifference <= (previousAccuracy / ACCURACY_PERCENT));

            final float[] results = new float[1];

            Location.distanceBetween(point.getLatitude(), point.getLongitude(), location.getLatitude(), location.getLongitude(), results);

            final float velocity = results[0] / ((location.getTime() - point.getTime()) / 1000);
            ...
            if ((velocity <= VELOCITY_THRESHOLD)
                    && ((currentAccuracy < previousAccuracy) || ((location.getTime() - lastPoint.getTime()) > TIME_THRESHOLD) || lowerAccuracyAcceptable)) {
                Log.d(TAG, "Adding point");
                onFilteredLocationChanged(context, location);
            }
            else {
    ...
    protected abstract void onFilteredLocationChanged(Context context, Location location);
}
