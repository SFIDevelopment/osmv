...
public class PoiOverlay extends BasePointOverlay implements RefreshableOverlay {
    ...
    public PoiOverlay(final Context ctx,
    ...
    public void setGpsStatusGeoPoint(final GeoPoint geopoint,
            final String title, final String descr) {
        final PoiPoint poi = new PoiPoint(title, descr, geopoint,
                R.drawable.poi);
        if (mItemList == null) {
    ...
    private class PoiListUpdater implements Runnable {
        ...
        public void run() {
            ...
            try {
                final List<PoiPoint> itemList = CoreInfoHandler
                        .getInstance()
                        .getDBManager(null)
                        .getPoiListNotHidden(mLastZoom, mLastMapCenter,
                                mdeltaX, mdeltaY);
                ...
                for (final PoiPoint point : itemList) {
                    ...
                    if (point.getIconId() <= 0) {
                        // get category & icon
                        final PoiCategory cat = CoreInfoHandler.getInstance()
                                .getDBManager(null)
                                .getPoiCategory(point.getCategoryId());
