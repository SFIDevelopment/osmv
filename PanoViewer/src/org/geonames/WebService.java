...
//    private static final String PANORAMIO_URL          = "http://www.panoramio.com/map/get_panoramas.php";
    ...
    private static InputStream connect(final String url) throws IOException {
        ...
        try {
            ...
            Log.e("WIMP","problems connecting to geonames server " + WebService.geoNamesServer + "Exception:" + e);
            ...
            Log.i("WIMP","trying to connect to failover server " + WebService.geoNamesServerFailover);
            // try failover server
    ...
    private static String getJSONStringValue(final JSONObject jsonObject, final String key) {
        ...
        try {
        	...
        	Log.d("WIMP","JSON Key [" + key + "] not found");
        }
        return value;
    }
    ...
    private static int getJSONIntValue(final JSONObject jsonObject, final String key) {
        ...
        try {
        	...
        	Log.e("WIMP","JSON Key [" + key + "] not found/valid");
        }
        return value;
    }
    ...
    private static WeatherObservation getWeatherObservationFromElement(final JSONObject weatherObservationElement) {
        ...
        try {
        	...
        	Log.e("WIMP","observationTime parsing failed");
        }
    ...
    private static List<WikipediaArticle> getWikiArticles(String url)throws Exception 
    {
    	final List<WikipediaArticle> articles = new ArrayList<WikipediaArticle>();
    ...
    public static List<WikipediaArticle> findNearbyWikipedia(final double latitude1, final double longitude1, final double latitude2, final double longitude2, final String language,
            final int maxRows) throws Exception {

        String url = "/wikipediaBoundingBoxJSON?";
    	
        url = url + "north=" + Math.max(latitude1,latitude2);
        url = url + "&south=" + Math.min(latitude1,latitude2);
        url = url + "east=" + Math.max(longitude1,longitude2);
        url = url + "&west=" + Math.min(longitude1,longitude2);
        if (maxRows > 0) {
            url = url + "&maxRows=" + maxRows;
        }
    ...
    private static Timezone getTimezoneFromJSON(final JSONObject json) {
        ...
        try {
            ...
            Log.d("WIMP","JSON date parsing failed");
        }
        try {
...
//
//        Log.d("WIMP",String.format("Searching Panoramio close to (%.6f, %.6f) Radius (m): %d\n", lat, lon, radius));
//
//        final double[] bbox = GeoMathUtil.boundingBoxCoords(lat, lon, radius);
//        Log.d("WIMP",String.format("Bounding box: Min: (%.6f, %.6f); Max:(%.6f, %.6f)\n\n", bbox[0], bbox[1], bbox[2], bbox[3]));
//        try {
...
//
//                    Log.d("WIMP",(i + 1) + ". ");
//                    Log.d("WIMP","Photo Title: " + item.getPhotoTitle());
//                    // Log.d("WIMP","   (lat,long): (%.6f,%,6f)\n"+
//                    // items[i].getLatitude()+" "+ items[i].getLongitude());
//                    Log.d("WIMP","   URL: " + item.getPhotoFileURL() + "\n");
//                }
//                return items;
//            }
//            else {
...
//    }
    ...
    public static GeoCodeResult reverseGeoCode(final double latitude, final double longitude) {
        ...
        try {
            ...
            Log.d("WIMP","Yahoo reverse geocoding failed");
        }
        return result;
    ...
    private static GeoCodeResult extractGeoCodeResult(final String yahooAnswer) {
        ...
        try {
            ...
            Log.d("WIMP","Yahoo Response not parsable...");
        }
        return result;
    }
