 ...
 * <li>Long press: Do nothing by default. Clients may use
 * {@link OnMapViewLongClickListener} to be notified of long presses on the
 * {@link PolarisMapView}</li>
 * <li>Double tap on callout: Zoom in/pan to the maximum level of detail
 * available</li>
 * </ul>
 * 
 * <h1>Effortless map annotating</h1>
 * <p>
 * {@link PolarisMapView} drastically simplifies addition of markers on a
 * {@link MapView} by getting rid of the {@link ItemizedOverlay} and introducing
 * {@link Annotation}s. {@link Annotation} is an extension of
 * {@link OverlayItem}. It contains map-related information such as coordinates
 * of a point, a title, a snippet and an optional marker Drawable. Annotating a
 * {@link PolarisMapView} consists on building a list of {@link Annotation}s and
 * adding it to the map with the {@link #setAnnotations(List, int)} or the
 * {@link #setAnnotations(List, Drawable)} method.
 * </p>
 * 
 * <h1>Automatic management of map callout</h1>
 * <p>
 * The Google Maps External Library includes an {@link OverlayItem} containing a
 * two Strings: a title and a snippet. Unfortunately, no matter how hard you
 * search into the documentation, you will notice these Strings are NEVER used,
 * ever. {@link PolarisMapView} automatically displays a map callout when the
 * underlying annotation is tapped. This makes addition of markers to a map
 * insanely easy.
 * </p>
 * 
 * <h1>Built-in "user tracking" button.</h1>
 * <p>
 * When user tracking is enabled, {@link PolarisMapView} automatically tracks
 * user location in the background (internally uses a {@link MyLocationOverlay})
 * and displays it on the map. The map can still be zoomed in and out and
 * panned. In addition to automatic tracking, a button letting the user
 * re-center the map on its current location is also overlaid in the top
 * right-hand corner.
 * </p>
 * 
 * <h1>Automatic built-in zoom controls</h1>
 * <p>
 * Android is used on a large set of devices. The vast majority of these devices
 * now supports multi-touch gestures. However, some other don't.
 * {@link PolarisMapView} seamlessly displays zoom controls when required (i.e.
 * the device doesn't support pinch-to-zoom). It prevents modern devices from
 * reducing the visible portion of the map with useless controls.
 * </p>
 * 
 * <h1>Map callout variable anchor positioning</h1>
 * <p>
 * Most (or should I say all) map-based applications uses 9-patches as map
 * callout background. While 9-patches are great in most cases, it doesn't allow
 * variable stretching of stretchable areas. In general, the Polaris library
 * contains default resources and more specifically {@link MapViewCallout}.
 * {@link MapViewCallout} allows variable positioning of the anchor. This
 * improvement is largely used by the Polaris library to get a more polished
 * map. While most applications center the map on the tapped {@link OverlayItem}, {@link PolarisMapView} shows a map callout trying to reduce scrolling as
 * much as possible. The map is actually scrolled only there is not enough space
 * to entirely show the map callout on screen.
 * </p>
 * 
 * <h1>Smooth showing/dismissing of map callouts</h1>
 * <p>
 * {@link PolarisMapView} animates showing and dismissing of map callouts. This
 * results in a more natural and smoother annotation rendering.
 * </p>
 * 
 * <h1>Additional listener</h1>
 * <p>
 * {@link PolarisMapView} lets you listen to region changes (i.e. when the map
 * has been zoomed and/or panned. Using {@link OnRegionChangedListener} can be
 * particularly useful in order to lazy load annotations depending on the
 * currently visible region displayed by the {@link PolarisMapView}
 * </p>
 * 
 * <p>
 * <strong>Note: </strong>{@link PolarisMapView} relies on {@link MapView} and
 * tries to be as transparent as possible. However there is a limitation when
 * adding {@link Overlay}s to the {@link PolarisMapView}. You MUST NOT use the
 * {@link MapView#getOverlays()} method and use the {@link #addOverlay(Overlay)}, {@link #removeOverlay(Overlay)} and their equivalents instead.
 * </p>
 * 
 * <p>
 * <strong>Note: </strong>Do not forget to call onStart() and onStop() in your
 * {@link MapActivity} equivalent. These methods binds the
 * {@link PolarisMapView} to the {@link MapActivity} lifecycle.
 * </p>
 * 
...
public class PolarisMapView extends MapView {
	...
	public interface OnRegionChangedListener {
		 ...
		 * 
		 * @param mapView
		 *            The {@link PolarisMapView} whose visible region changed
	 ...
	 * @author Cyril Mottier
	 */
	public static class OnMapViewRegionChangedListenerAdapter implements
	...
	public interface OnAnnotationSelectionChangedListener {
		 ...
		 * 
		 * @param mapView
		 *            The {@link MapView} whose annotation has been selected
		 * @param calloutView
		 *            The {@link View} that is used as a callout for this
		 *            annotation
		 * @param position
		 *            The position of the annotation in the annotations list
		 * @param annotation
		 *            The annotation associated to the tapped marker
		 */
		void onAnnotationSelected(PolarisMapView mapView,
		 ...
		 * 
		 * @param mapView
		 *            The {@link MapView} whose annotation has been de-selected
		 * @param calloutView
		 *            The {@link View} that is used as a callout for this
		 *            annotation
		 * @param position
		 ...
		 *            The annotation
		 */
		void onAnnotationDeselected(PolarisMapView mapView,
				MapCalloutView calloutView, int position, Annotation annotation);
		 ...
		 * 
		 * @param mapView
		 *            The {@link MapView} whose annotation has been clicked
		 * @param calloutView
		 *            The {@link View} that is used as a callout for this
		 *            annotation
		 * @param position
	 ...
	 * @author Cyril Mottier
	 */
	public static class OnAnnotationSelectionChangedListenerAdapter implements
	 ...
	 * 
	 * @param index
	 *            The index at which to insert the {@link Overlay}
	 * @param overlay
	 *            The {@link Overlay} to add
	 */
	 ...
	 * 
	 * @param index
	 *            The index of the {@link Overlay} to remove.
	 */
	 ...
	 * 
	 * @param listener
	 *            The new {@link OnRegionChangedListener}
	 */
	public void setOnRegionChangedListenerListener(
	 ...
	 * 
	 * @param listener
	 *            The new {@link OnMapViewLongClickListener}
	 */
	 ...
	 * 
	 * @param enabled
	 *            True if user tracking is enabled, false otherwise.
	 */
		...
		if (mIsUserTrackingButtonEnabled != enabled) {
			...
			if (enabled) {
				if (mUserTrackingButton == null) {
					...
					mUserTrackingButton
							.setOnClickListener(mOnUserTrackingButtonClickListener);
					// @formatter:off
					mUserTrackingButton
	 ...
	 * 
	 * @param actionnable
	 *            true to make the view actionnable, false otherwise
	 */
	 ...
	 * 
	 * @param listener
	 *            The new {@link OnAnnotationSelectionChangedListener}
	 */
	public void setOnAnnotationSelectionChangedListener(
			OnAnnotationSelectionChangedListener listener) {
	 ...
	 * @param annotations
	 *            The annotations
	 * @param annotationMarkerId
	 *            The resource identifier to the default marker.
	 */
	public void setAnnotations(List<Annotation> annotations,
			int annotationMarkerId) {
		setAnnotations(annotations,
				getResources().getDrawable(annotationMarkerId));
	}
	 ...
	 * @param annotations
	 *            The annotations
	 * @param annotationMarker
	 *            The default marker
	 */
	public void setAnnotations(List<Annotation> annotations,
			Drawable annotationMarker) {
		if (annotations == null) {
			mOverlayContainer.setAnnotationsOverlay(null);
		} else {
			mAnnotationsOverlay = new AnnotationsOverlay(mMystiqueCallback,
					new ArrayList<Annotation>(annotations), annotationMarker);
	...
	public Annotation getSelectedAnnotation() {
		if (mAnnotationsOverlay != null) {
			return mAnnotationsOverlay.getAnnotation(mAnnotationsOverlay
	 ...
	 * 
	 * @param position
	 *            The position of the annotation to select
	 */
	...
	public void getCoordinateRegion(CoordinateRegion region) {
		// @formatter:off
		region.set(getMapCenter().getLatitudeE6(), getMapCenter()
				.getLongitudeE6(), getLatitudeSpan(), getLongitudeSpan());
		// @formatter:on
	}
	...
	private void scheduleRegionChangeConfirmed() {
		if (!mIsInGesture) {
			removeCallbacks(mRegionChangeConfirmedRunnable);
			postDelayed(mRegionChangeConfirmedRunnable,
					REGION_CHANGE_CONFIRMED_DELAY);
		}
	}
	...
	private final Runnable mRegionChangeConfirmedRunnable = new Runnable() {
		public void run() {
			if (mOnRegionChangedListener != null && testRegionChangeConfirmed()) {
				mOnRegionChangedListener
						.onRegionChangeConfirmed(PolarisMapView.this);
			}
		}
	};
	...
	private final MystiqueCallback mMystiqueCallback = new MystiqueCallback() {
		...
		public void dismissCallout(int position) {
			final Annotation annotation = mAnnotationsOverlay
					.getAnnotation(position);
			if (annotation == null) {
				return;
			}
			final MapCalloutView mapCalloutView = getCurrentMapCallout();
			if (mapCalloutView != null
					&& mapCalloutView.getVisibility() == View.VISIBLE) {
				mapCalloutView.dismiss(true);
				if (mOnAnnotationSelectionChangedListener != null) {
					// @formatter:off
					mOnAnnotationSelectionChangedListener
							.onAnnotationDeselected(PolarisMapView.this,
									mapCalloutView, position, annotation);
					// @formatter:on
				}
			}
		}
		...
		public void showCallout(int position) {
			...
			int markerHeight = mAnnotationsOverlay.getDefaultMarker()
					.getBounds().height();
			...
			if (mOnAnnotationSelectionChangedListener != null) {
				...
				mOnAnnotationSelectionChangedListener.onAnnotationSelected(
						PolarisMapView.this, mapCalloutView, position,
						annotation);
				// @formatter:on
			}

			if (mapCalloutView.hasDisplayableContent()) {
				mapCalloutView.show(PolarisMapView.this, annotation.getPoint(),
						true);
			}
	...
	private final MagnetoCallback mMagnetoCallback = new MagnetoCallback() {
		...
		public void onLongPress(MotionEvent e) {
			if (mOnMapViewLongClickListener != null) {
				mOnMapViewLongClickListener.onLongClick(
						PolarisMapView.this,
	...
	private final OnClickListener mOnClickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			if (mOnAnnotationSelectionChangedListener != null) {
				...
				if (annotation != null
						&& annotationPosition != INVALID_POSITION) {
					// @formatter:off
					mOnAnnotationSelectionChangedListener.onAnnotationClicked(
							PolarisMapView.this, (MapCalloutView) v,
	...
	private final OnClickListener mOnUserTrackingButtonClickListener = new OnClickListener() {
		@Override
		public void onClick(View v) {
			MapViewUtils.smoothCenterOnUserLocation(PolarisMapView.this,
					mMyLocationOverlay, R.string.polaris__unable_to_locate_you);
		}
	};

	public void addAnnotation(Annotation annotation) {
		if (mAnnotationsOverlay != null) {
			mAnnotationsOverlay.addAnnotation(annotation);
		}
