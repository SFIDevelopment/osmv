...
public class WerteListFragment extends SherlockListFragment {
	...
	@Override
	public void onPause() {
		NotificationManager.deregister(this);
	...
	void restoreSavedState(final Bundle savedInstanceState) {
	...
	public void onListItemClick(final ListView l, final View v,
		...
		final String text = makeText(getActivity(),entries,activeSite,  entries.size() - 1, false);
	...
	void shareIt(final String text) {
	...
	public static String makeText(Context context,List<WindEntry> entries,final int activeSite, final int index, final boolean forSpeach) {
		...
		// header
		String text = context.getText(R.string.tts_weather_station).toString()
				+ " "
				+ context.getResources().getStringArray(R.array.site_url_title)[activeSite];
		...
		text += "  "
				+ context.getText(R.string.tts_windspeed).toString()
				+ " "
				...
				+ "  "
				+ context.getText(R.string.tts_windspeed_max).toString()
				+ " "
				...
				+ context.getResources().getStringArray(
						forSpeach ? R.array.speed_unit_title_tts
								: R.array.speed_unit_title)[conversionIx]
				+ "  "
				+ context.getText(R.string.tts_winddir).toString()
				+ " "
				+ entry.getValue(3)
				+ " "
				+ context.getText(R.string.tts_grad).toString()
				+ "  "
				+ context.getResources().getStringArray(R.array.wind_directions)[WindInfoHandler
						.getArrowIndex(Integer.parseInt(entry.getValue(3)))];
		// temp
		text += "  " + context.getText(R.string.tts_temp).toString() + " "
	...
	private void speakit() {
		...
		if ((entries != null) && (entries.size() > 0)) {
			if ((refresh) && (CoreInfoHolder.getInstance().isSpeakit())) {
				SpeakItOut.speak(makeText(getActivity(),entries,activeSite,  entries.size() - 1, true));
			}
		} else {
	...
	public List<WindEntry> getEntries() {
		return entries;
	}
	...
	void onPostExecuteGetData(final List<WindEntry> entries) {
		...
		final ListAdapter adapter = new BaseAdapter() {
			...
			private final int[] textFieldIds = { R.id.text1, R.id.text2,
					R.id.text3, R.id.text4, R.id.text5, R.id.text6, R.id.text7 };
			...
			public View getView(int position, View convertView,
				...
				for (int i = 0; i < holder.textView.length; i++) {
					...
					if (windEntry.getValue(i) != null) {
						if ((i == 2) || (i == 4)) {
							final double speed = WindInfoHandler.convertSpeed(
									Double.parseDouble(windEntry.getValue(i)
											.replace(",", ".")), conversionIx);
				...
				holder.textView[4].setTextColor(WindInfoHandler
						.getSpeedColor(Double.parseDouble(windEntry.getValue(4)
								.replace(",", "."))));

				holder.textView[2].setTextColor(WindInfoHandler
						.getSpeedColor(Double.parseDouble(windEntry.getValue(2)
								.replace(",", "."))));
