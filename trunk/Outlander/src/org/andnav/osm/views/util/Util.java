...
public class Util implements OpenStreetMapViewConstants {
    ...
    public static int[] getMapTileFromCoordinates(final double aLat,
        ...
        if (aProjection == 3) {
            ...
            if (aProjection == 1) {
                out[OpenStreetMapViewConstants.MAPTILE_LATITUDE_INDEX] = (int) Math
                        .floor(((1 - (Math.log(Math.tan((aLat * Math.PI) / 180)
                ...
                final double M2 = (Math.log((1 + Math.sin(E2))
                        / (1 - Math.sin(E2))) / 2)
                        - ((J2 * Math.log((1 + (J2 * Math.sin(E2)))
                                / (1 - (J2 * Math.sin(E2))))) / 2);
                final double B2 = (1 << zoom);
                out[OpenStreetMapViewConstants.MAPTILE_LATITUDE_INDEX] = (int) Math
                        .floor((B2 / 2) - ((M2 * B2) / 2 / Math.PI));
            }

            out[OpenStreetMapViewConstants.MAPTILE_LONGITUDE_INDEX] = (int) Math
                    .floor(((aLon + 180) / 360) * (1 << zoom));
        }
    ...
    public static BoundingBoxE6 getBoundingBoxFromMapTile(final int[] aMapTile,
        ...
        if (aProjection == 3) {
            final double[] LatLon0 = OSGB36
                    .OSGB2LatLon(
                            (((OpenStreetMapViewConstants.OpenSpaceUpperBoundArray[zoom - 7]
                                    - y - 1) * 1000000) / OpenStreetMapViewConstants.OpenSpaceUpperBoundArray[zoom - 7]),
                            ((x * 1000000) / OpenStreetMapViewConstants.OpenSpaceUpperBoundArray[zoom - 7]));
            final double[] LatLon1 = OSGB36
                    .OSGB2LatLon(
                            ((((OpenStreetMapViewConstants.OpenSpaceUpperBoundArray[zoom - 7]
                                    - y - 1) + 1) * 1000000) / OpenStreetMapViewConstants.OpenSpaceUpperBoundArray[zoom - 7]),
                            (((x + 1) * 1000000) / OpenStreetMapViewConstants.OpenSpaceUpperBoundArray[zoom - 7]));
            return new BoundingBoxE6(LatLon1[0], LatLon1[1], LatLon0[0],
                    LatLon0[1]);
        } else {
            return new BoundingBoxE6(tile2lat(y, zoom, aProjection), tile2lon(
                    x + 1, zoom), tile2lat(y + 1, zoom, aProjection), tile2lon(
                    x, zoom));
        }
    }
    ...
    private static double tile2lat(final int y, final int aZoom,
        ...
        if (aProjection == 1) {
            ...
            double Zum1 = Zu;
            Zu = Math.asin(1 - (((1 + Math.sin(Zum1)) * Math.pow(
                    1 - (FExct * Math.sin(Zum1)), FExct)) / (Math.exp((2 * yy)
                    / -(TilesAtZoom / (2 * Math.PI))) * Math.pow(
                    1 + (FExct * Math.sin(Zum1)), FExct))));
            while (Math.abs(Zum1 - Zu) >= MerkElipsK) {
                Zum1 = Zu;
                Zu = Math.asin(1 - (((1 + Math.sin(Zum1)) * Math.pow(
                        1 - (FExct * Math.sin(Zum1)), FExct)) / (Math
                        .exp((2 * yy) / -(TilesAtZoom / (2 * Math.PI))) * Math
    ...
    public static double y2lat(final int y, final int aZoom,
            final int MAPTILE_SIZEPX) {
        // final int aProjection = 1;

        // if (aProjection == 1) {
        final double n = Math.PI
                - (((2.0 * Math.PI * y) / MAPTILE_SIZEPX) * Math
                        .pow(2.0, aZoom));
        return (180.0 / Math.PI)
                * Math.atan(0.5 * (Math.exp(n) - Math.exp(-n)));
        // } else {
