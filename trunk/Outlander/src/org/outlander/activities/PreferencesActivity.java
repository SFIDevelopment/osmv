...
public class PreferencesActivity extends SherlockPreferenceActivity implements
        OnSharedPreferenceChangeListener, PrefConstants {
            ...
            if (files != null) {
                for (final File file : files) {
                    if (file.getName().toLowerCase()
                            .endsWith(getString(R.string.mnm))
                            || file.getName().toLowerCase()
                                    .endsWith(getString(R.string.tar))
                            || file.getName().toLowerCase()
                                    .endsWith(getString(R.string.sqlitedb))) {
                        final String name = Ut.FileName2ID(file.getName());

                        final PreferenceScreen prefscr = getPreferenceManager()
                                .createPreferenceScreen(this);
                        prefscr.setKey(PrefConstants.PREF_USERMAPS_ + name);
                        {
                            final CheckBoxPreference pref = new CheckBoxPreference(
                                    this);
                            pref.setKey(PrefConstants.PREF_USERMAPS_ + name
                                    + "_enabled");
                        ...
                        {
                            final EditTextPreference pref = new EditTextPreference(
                                    this);
                            pref.setKey(PrefConstants.PREF_USERMAPS_ + name
                                    + "_name");
                        ...
                        prefscr.setTitle(prefscr.getSharedPreferences()
                                .getString(
                                        PrefConstants.PREF_USERMAPS_ + name
                                                + "_name", file.getName()));
                        if (prefscr.getSharedPreferences().getBoolean(
                                PrefConstants.PREF_USERMAPS_ + name
                                        + "_enabled", false)) {
                            prefscr.setSummary("Enabled  "
                                    + file.getAbsolutePath());
                        } else {
                            prefscr.setSummary("Disabled  "
                                    + file.getAbsolutePath());
                        }
    ...
    public void onSharedPreferenceChanged(final SharedPreferences aPref,
        ...
        if (aKey.equalsIgnoreCase("pref_dir_maps")) {
            findPreference("pref_main_useTschekkoMaps").setSummary(
                    "Maps from "
                            + aPref.getString("pref_dir_maps", Environment
                                    .getExternalStorageDirectory().getPath()
                                    + "/TschekkoMaps/maps/"));
            findPreference(aKey).setSummary(
                    aPref.getString("pref_dir_maps", Environment
                            .getExternalStorageDirectory().getPath()
                            + "/TschekkoMaps/maps/"));

            final File dir = new File(aPref
                    .getString(
                            "pref_dir_maps",
                            Environment.getExternalStorageDirectory().getPath()
                                    + "/TschekkoMaps/maps/").concat("/")
                    .replace("//", "/"));
            if (!dir.exists()) {
            ...
            findPreference("pref_dir_main").setSummary(
                    aPref.getString("pref_dir_main", Environment
                            .getExternalStorageDirectory().getPath()
                            + "/TschekkoMaps/"));
            findPreference("pref_dir_import").setSummary(
                    aPref.getString("pref_dir_import", Environment
                            .getExternalStorageDirectory().getPath()
                            + "/TschekkoMaps/import/"));
            findPreference("pref_dir_export").setSummary(
                    aPref.getString("pref_dir_export", Environment
                            .getExternalStorageDirectory().getPath()
                            + "/TschekkoMaps/export/"));
        } else if (aKey.length() > 14) {
            if (aKey.substring(0, 14).equalsIgnoreCase(
                    PrefConstants.PREF_USERMAPS_)) {
                if (aKey.endsWith("name") && (findPreference(aKey) != null)) {
                    findPreference(aKey).setSummary(aPref.getString(aKey, ""));
                    findPreference(aKey.replace("_name", "")).setTitle(
                            aPref.getString(aKey, ""));
                } else if (aKey.endsWith("enabled")
                        && (findPreference(aKey.replace("_enabled", "")) != null)) {
                    if (aPref.getBoolean(aKey, false)) {
                        findPreference(aKey.replace("_enabled", ""))
                                .setSummary(
                                        "Enabled  "
                                                + aPref.getString(
                                                        aKey.replace(
