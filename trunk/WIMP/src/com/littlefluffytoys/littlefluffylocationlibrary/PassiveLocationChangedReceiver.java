...
public class PassiveLocationChangedReceiver extends BroadcastReceiver {
	...
	protected static void processLocation(final Context context,
		...
		if (lastLat != Long.MIN_VALUE) {
			...
			Location.distanceBetween(lastLat, lastLong, thisLat, thisLong,
					distance);

			final int distanceBetweenInMetres = (int) distance[0];
		...
		if (!usePreviousReading) {
			...
			prefsEditor
					.putInt(LocationLibraryConstants.SP_KEY_LAST_LOCATION_UPDATE_ACCURACY,
							thisAccuracy);
			if (LocationLibrary.showDebugOutput) {
				Util.d(TAG + ": Storing location update, lat=" + thisLat
						+ " long=" + thisLong + " accuracy=" + thisAccuracy
						+ " time=" + thisTime + "("
