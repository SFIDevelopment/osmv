...
public class PassiveLocationChangedReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (intent
                .hasCategory(LocationLibraryConstants.INTENT_CATEGORY_ONE_SHOT_UPDATE)) {
            ...
            if (LocationLibrary.showDebugOutput)
                Log.d(LocationLibraryConstants.TAG, TAG
                        + ":onReceive: on-demand location update received");

            // we know that the passive provider will shortly get an update from
            // this.
            ...
            LocationBroadcastService.forceDelayedServiceCall(context, 30,LocationBroadcastService.class);
            ...
            if (LocationLibrary.showDebugOutput)
                Log.w(LocationLibraryConstants.TAG, TAG
                        + ":onReceive: Unknown update received");
        }
    }

    protected static void processLocation(final Context context,
            final Location location) {
        final SharedPreferences prefs = PreferenceManager
                .getDefaultSharedPreferences(context);
        final float lastLat = prefs.getFloat(
                LocationLibraryConstants.SP_KEY_LAST_LOCATION_UPDATE_LAT,
                Long.MIN_VALUE);
        final float lastLong = prefs.getFloat(
                LocationLibraryConstants.SP_KEY_LAST_LOCATION_UPDATE_LNG,
                Long.MIN_VALUE);
        final int lastAccuracy = prefs.getInt(
        ...
        if (lastLat != Long.MIN_VALUE) {
            // The tricky maths bit to calculate the distance between two
            // points:
            // dist = arccos(sin(lat1) · sin(lat2) + cos(lat1) · cos(lat2) ·
            // cos(lon1 - lon2)) · R
            int distanceBetweenInMetres = (int) (Math.acos(Math.sin(Math
                    .toRadians(thisLat))
                    ...
                    * Math.cos(Math.toRadians(thisLong)
                            - Math.toRadians(lastLong))) * 6371 * 1000);
            if (LocationLibrary.showDebugOutput)
                Log.d(LocationLibraryConstants.TAG, TAG
                        + ": Distance from last reading: "
                        + distanceBetweenInMetres + "m");
        ...
        if (!usePreviousReading) {
            ...
            prefsEditor.putString(
                    LocationLibraryConstants.SP_KEY_LAST_LOCATION_UPDATE_PROVIDER,
                    thisProvider);
            prefsEditor
                    .putInt(LocationLibraryConstants.SP_KEY_LAST_LOCATION_UPDATE_ACCURACY,
                            thisAccuracy);
            if (LocationLibrary.showDebugOutput)
                Log.d(LocationLibraryConstants.TAG,
                        TAG + ": Storing location update, lat=" + thisLat
                                + " long=" + thisLong + " accuracy="
                                + thisAccuracy + " time=" + thisTime + "("
                                + DateFormat.format("kk:mm.ss, E", thisTime)
                                + ")");
        } else {
            if (LocationLibrary.showDebugOutput)
                Log.d(LocationLibraryConstants.TAG,
                        TAG
                                + ": Storing location update, less accurate so reusing prior location - time="
                                + thisTime);
        }
        prefsEditor.commit();
