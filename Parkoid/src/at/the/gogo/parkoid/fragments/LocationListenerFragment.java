...
public abstract class LocationListenerFragment extends SherlockFragment
    ...
    public void onCreateContextMenu(final ContextMenu menu, final View v,
        ...
        if (parkingLotAvailable()) {
            menu.add(0, R.id.navigateToCar, 0,
                    getText(R.string.menu_navigateToCar));
        ...
        if (kpzStateChange) {
            if (CoreInfoHolder.getInstance().isSpeakit()) {
                SpeakItOut.speak(getText(
                        (inZone) ? R.string.tts_near_kpz1
                                : R.string.tts_no_near_kpz).toString());

                // only if not in motion
                if ((CoreInfoHolder.getInstance().getLastKnownLocation() != null)
                        && (CoreInfoHolder.getInstance().getLastKnownLocation()
                                .hasSpeed())) {

                    if (CoreInfoHolder.getInstance().getLastKnownLocation()
                            .getSpeed() < 10) { // TODO: to be evaluated
                        if (CoreInfoHolder.getInstance().isSpeakit()) {
                            SpeakItOut.speak(getText(
                                    (inZone) ? R.string.tts_near_kpz2
                                            : R.string.tts_no_near_kpz)
                                    .toString());
                        }
                    }
                }
    ...
    private Position getDefaultCarLocation() {
        Position carLocation = null;

        final List<CarPosition> locations = CoreInfoHolder.getInstance()
                .getDbManager().getLastLocationsList();
        if ((locations != null) && (locations.size() > 0)) {
            carLocation = locations.get(locations.size() - 1);
        }

        return carLocation;
    }

    protected boolean parkingLotAvailable() {
        return (getDefaultCarLocation() != null);
    }

    protected void navigateToCar() {

        final Position carLocation = getDefaultCarLocation();
        ...
        if (carLocation != null) {
            ...
            final DialogFragment newFragment = RoutingDialogFragment
                    .newInstance("Parkplatz", CoreInfoHolder.getInstance()
                            .getLastKnownLocation().getLatitude(),
                            CoreInfoHolder.getInstance().getLastKnownLocation()
                                    .getLongitude(), carLocation.getLatitude(),
                            carLocation.getLongitude(),
                            R.string.menu_navigateToCar);
